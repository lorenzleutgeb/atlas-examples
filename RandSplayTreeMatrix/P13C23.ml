 ...
 * DO NOT EDIT
 *
 * Cost for recursion : 2/3 = 1 - 1/3
 * Cost for rotation  : 1/3 = 1 - 2/3
 * Coin probability   : 1/3
 *)

splay ∷ Ord α ⇒ (α ⨯ Tree α) → Tree α | [[0 ↦ 5/6, (1 0) ↦ 5/4, (0 2) ↦ 2/3] → [0 ↦ 5/6, (0 2) ↦ 2/3], {[(1 0) ↦ 5/12] → [(1 0) ↦ 5/12]}]
        ...
        | node bl b br → if a == b
          then (node (node bl a br) c cr)  (* No rotation! *)
            ...
            then match bl with
              ...
              | bl   → match ~ 2/3 splay a bl with
                | node al a1 ar → if coin 1/3
                  then ~ 1/3 (node al a1 (node ar b (node br c cr)))
                  else (node (node (node al a1 ar) b br) c cr)
            else match br with
              | leaf → (node bl b (node leaf c cr))
              | br   → match ~ 2/3 splay a br with
                | node al a1 ar → if coin 1/3
                  then ~ 1/3 (node (node bl b al) a1 (node ar c cr))
