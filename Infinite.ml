...
infinite_5 x t = (let s = t in (node (~ infinite_5 x t) x t))

infinite_6 ∷ α ⨯ Tree α ⨯ Tree α → Tree α
infinite_6 x t1 t2 = (node ((~ infinite_2 x t1)) x t2)

infinite_7 ∷ α ⨯ Tree α ⨯ Tree α → Tree α
infinite_7 x t1 t2 = (let s = t1 in (node ((~ infinite_2 x s)) x t2))

infinite_8 ∷ α ⨯ Tree α → Tree α
infinite_8 x t = (let s = t in (node (~ infinite_8 x s) x s))

infinite_9 ∷ α ⨯ Tree α ⨯ Tree α → Tree α
infinite_9 x t1 t2 = (let s = t2 in (node (~ infinite_9 x s t1) x t1))
...
infinite_12 x t = (let s = t in (node (~ infinite_12 x s) x leaf))

infinite_13 ∷ α ⨯ Tree α ⨯ Tree α → Tree α
infinite_13 x t u = (let s = t in (node (~ infinite_13 x s leaf) x u))

infinite_14 ∷ α → Tree α
infinite_14 x = (node (~ infinite_14 x) x leaf)
...
infinite_19a x t = (node (~ infinite_19b x t) x leaf)
